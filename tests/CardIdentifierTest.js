...
const   assertContainsNoIdentifications = require('./BaseTestCase.js').assertContainsNoIdentifications,
     ...
     * @return {Array}
     */
    function providerInspectTextWithNotices(){
        return [
            ['1.1. Text contains invalid numbers (none)', 'foo bar bla bla', null],
            ['1.2. Text contains invalid numbers (continuous)', 'foo 12345 bla bla', null],
            ['1.3. Text contains invalid numbers (discontinuous)', 'foo 12 34 56 bla bla', null],
     ...
     * @return {Array}
     */
    function providerInspectTextWithNoticesWithoutThreshold(){
        return [
            ['1.1. Text contains invalid numbers (none)', 'foo bar bla bla', null],
            ['1.2. Text contains invalid numbers (continuous)', 'foo 12345 bla bla', null],
            ['1.3. Text contains invalid numbers (discontinuous)', 'foo 12 34 56 bla bla', null],
     ...
     */
    describe('inspectText', function(){
        providerInspectText().forEach(function (row){
            const testName = row[0];
            const testText = row[1];
            const expectedResults = row[2];
            {
                const identifier = new app.CardIdentifier();
                const identifiedText = identifier.inspectText(testText);
                if (expectedResults === null) {
                    assertContainsNoIdentifications(identifiedText);
                }
                else {
                    expectedResults.forEach(function (expectedResult) {
                        it('should contain expected Result for Test '+testName, function(){
                            assert.include(identifiedText, expectedResult);
                        });
                    });
                }
            }
     ...
     */
    describe('inspectTextWithNotices', function(){
        providerInspectTextWithNotices().forEach(function (row){
            const testName = row[0];
            const testText = row[1];
            const expectedResults = row[2];
            {
                const identifier = new app.CardIdentifier(85,20);
                const identifiedText = identifier.inspectTextWithNotices(testText);
                if (expectedResults === null) {
                    assertContainsNoIdentifications(identifiedText);
                }
                else {
     ...
     */
    describe('inspectTextWithNoticesWithoutThreshold', function(){
        providerInspectTextWithNoticesWithoutThreshold().forEach(function (row){
            const testName = row[0];
            const testText = row[1];
            const expectedResults = row[2];
            {
                const identifier = new app.CardIdentifier(85);
                const identifiedText = identifier.inspectTextWithNotices(testText);
                if (expectedResults === null) {
                    assertContainsNoIdentifications(identifiedText);
                }
